...
func RunSurfaceDiscovery(
	...
	//fuzzy search domains
	{
		...
		untestedFuzzDomains, err := Alterx(pipeline.Domains)
		if err != nil {
			logger.Error("alterx fail", "error", err)
			return
		}
		logger.Info("pipeline - after fuzz", "domains", untestedFuzzDomains)

		// validate fuzzed domains
		// note that at this stage these domains could be false positives
		// generated by wildcard domains
		fuzzDomains := DnsxFilterActive(untestedFuzzDomains, dnsCache)

		logger.Info("pipeline - fuzz active dns", "domains", fuzzDomains)
